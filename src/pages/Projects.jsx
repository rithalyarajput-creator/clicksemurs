  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }} onClick={onClose}>
      ...
      <div style={{ background: '#fff', borderRadius: 20, overflow: 'hidden', maxWidth: 860, width: '100%', maxHeight: '90vh', overflowY: 'auto', position: 'relative', zIndex: 1 }} onClick={e => e.stopPropagation()}>
        {/* Header */}
        <div style={{ padding: '20px 28px', borderBottom: '1px solid rgba(0,0,0,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div>
              <h2 style={{ fontSize: 20, fontWeight: 800, color: '#1a1a1a', marginBottom: 2 }}>{project.title}</h2>
              <span style={{ fontSize: 12, fontWeight: 700, color: '#c8892a', textTransform: 'uppercase', letterSpacing: '0.08em' }}>{project.category}</span>
            </div>
          </div>
          <button onClick={onClose} style={{ background: 'rgba(0,0,0,0.06)', border: 'none', borderRadius: '50%', width: 36, height: 36, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        ...
        <div style={{ padding: 28 }}>
          ...
          {images.length > 0 && (
            <div style={{ marginBottom: 16 }}>
              <div style={{ borderRadius: 12, overflow: 'hidden', background: '#fefaef', marginBottom: 10 }}>
              ...
              {images.length > 1 && (
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {images.map((img, i) => (
                    <button key={i} onClick={() => setActiveImg(i)} style={{ width: 72, height: 52, borderRadius: 8, overflow: 'hidden', border: activeImg === i ? '2px solid #1a1a1a' : '2px solid transparent', cursor: 'pointer', padding: 0, background: 'none' }}>
          ...
          {project.description && (
            <p style={{ color: '#555', fontSize: 15, lineHeight: 1.7 }}>{project.description}</p>
          )}
        </div>
      </div>
    </div>
...
export default function Projects() {
  ...
  return (
    <div style={{ overflowX: 'hidden' }}>
      ...
      {/* Hero */}
      <section style={{ background: 'linear-gradient(135deg, #1a1a1a 0%, #111827 60%, #1a1a1a 100%)', padding: 'clamp(152px,14vw,196px) 0 clamp(56px,6vw,72px)', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', inset: 0, backgroundImage: 'linear-gradient(rgba(255,255,255,0.025) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.025) 1px, transparent 1px)', backgroundSize: '60px 60px' }} />
        <div style={{ position: 'absolute', right: 0, top: 0, width: '50%', height: '100%', backgroundImage: 'radial-gradient(ellipse at 80% 50%, rgba(200,137,42,0.08) 0%, transparent 60%)', pointerEvents: 'none' }} />
        <div className="max-w-7xl mx-auto px-6 lg:px-8" style={{ position: 'relative', textAlign: 'center' }}>
          ...
          <h1 style={{ fontSize: 'clamp(2rem, 6vw, 3.8rem)', fontWeight: 900, color: '#fefaef', lineHeight: 1.1, marginBottom: 20, letterSpacing: '-0.02em' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 10, flexWrap: 'wrap' }}>
            {CATEGORIES.filter(c => catCounts[c] > 0).map(c => (
              <div key={c} style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 100, padding: '6px 16px', fontSize: 12, color: '#aaaaaa', fontWeight: 600 }}>
      ...
      {/* Filter + Grid */}
      <section style={{ background: '#fefaef', padding: 'clamp(48px,6vw,72px) 0' }}>
        <div className="max-w-7xl mx-auto px-6 lg:px-8">
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 40 }}>
            {CATEGORIES.map(c => (
              <button key={c} onClick={() => setActiveCategory(c)} style={{
                ...
                border: activeCategory === c ? '1px solid #1a1a1a' : '1px solid rgba(0,0,0,0.1)',
                background: activeCategory === c ? '#1a1a1a' : '#fff',
                color: activeCategory === c ? '#fefaef' : '#666',
          ...
          {/* Grid */}
          {loading ? (
            <div style={{ textAlign: 'center', padding: 60, color: '#aaaaaa' }}>Loading projects...</div>
          ) : filtered.length === 0 ? (
            <div style={{ textAlign: 'center', padding: 60, color: '#aaaaaa' }}>
              ...
              {filtered.map((p, i) => {
                ...
                return (
                  <FadeIn key={p.id} delay={i * 60}>
                    <div
                      style={{ background: '#fff', borderRadius: 16, overflow: 'hidden', border: '1px solid rgba(0,0,0,0.08)', cursor: 'pointer', transition: 'box-shadow 0.2s, transform 0.2s' }}
                    ...
                    >
                      {/* Image area */}
                      <div style={{ height: 220, background: '#f5e6c8', position: 'relative', overflow: 'hidden' }}>
                        {p.image1 ? (
                        ...
                        <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0)', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.2s' }}
                          ...
                          <span style={{ background: '#fff', color: '#1a1a1a', fontSize: 12, fontWeight: 700, padding: '8px 18px', borderRadius: 100, opacity: 0, transition: 'opacity 0.2s', pointerEvents: 'none' }}>
                            View Project
                          </span>
                        </div>
                      </div>
                      ...
                      <div style={{ padding: '18px 20px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                          ...
                          <h3 style={{ fontWeight: 800, fontSize: 16, color: '#1a1a1a', lineHeight: 1.3, flex: 1 }}>{p.title}</h3>
                        </div>
                        {p.description && (
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4, color: '#1a1a1a', fontSize: 13, fontWeight: 700 }}>
      ...
      {/* CTA */}
      <section style={{ background: '#1a1a1a', padding: 'clamp(56px,6vw,80px) 0', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="max-w-3xl mx-auto px-6 lg:px-8" style={{ textAlign: 'center', position: 'relative' }}>
          <h2 style={{ fontSize: 'clamp(1.6rem, 4vw, 2.4rem)', fontWeight: 900, color: '#fefaef', marginBottom: 14, letterSpacing: '-0.02em' }}>
            Want a Project Like This?
          </h2>
          <p style={{ color: '#aaaaaa', fontSize: 16, marginBottom: 32, lineHeight: 1.7 }}>Let's build something remarkable together.</p>
          <Link to="/contact" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: '#c8892a', color: '#fff', padding: '14px 32px', fontWeight: 800, fontSize: 14, letterSpacing: '0.06em', textTransform: 'uppercase', textDecoration: 'none', borderRadius: 8 }}>
