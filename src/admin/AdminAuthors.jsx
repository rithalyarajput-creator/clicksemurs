      ...
      <div style={{ display: 'grid', gridTemplateColumns: '340px 1fr', gap: 24, alignItems: 'start' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 16 }}>
          {authors.length === 0 ? (
            ...
            <div key={a.id} style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 12, padding: 20, display: 'flex', gap: 14, alignItems: 'flex-start' }}>
              <img src={a.avatar || 'https://ui-avatars.com/api/?name=' + a.name + '&background=0f172a&color=fff'} alt={a.name}
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: 'flex', gap: 6, marginTop: 10 }}>
                  <button onClick={() => { setForm({ name: a.name, bio: a.bio || '', avatar: a.avatar || '' }); setEditId(a.id) }}
                    style={{ background: '#eff6ff', border: 'none', borderRadius: 6, padding: '4px 10px', fontSize: 11, cursor: 'pointer', color: '#2563eb' }}>✏️ Edit</button>
                  <button onClick={() => del(a.id)} style={{ background: '#fef2f2', border: 'none', borderRadius: 6, padding: '4px 10px', fontSize: 11, cursor: 'pointer', color: '#dc2626' }}>🗑 Delete</button>
                </div>
              </div>
            </div>
          ))}
